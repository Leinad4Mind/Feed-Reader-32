...
class sfnc_class
{
	...
	private function get_file()
	{
		...
		if (!$content)
		{
			...
			$phpbb_log->add('critical', $user->data['user_id'], $user->ip, 'LOG_ERROR_SFNC_ERROR_NO_CONTENT', false, array($this->url));
		}
	...
	private function parse_data()
	{
		...
		else
		{
			// TODO add lang entry to error log lang file
			global $phpbb_log, $user;
			$phpbb_log->add('critical', $user->data['user_id'], $user->ip, 'LOG_ERROR_SFNC_NO_FEED_TYPE', false, array($this->name));
		}
	}
		...
		if ($this->cron_init || ($this->index_init && ($this->next_update < time())))
		{
			if (!preg_match('/^(http|https):\/\/([A-Z0-9][A-Z0-9_-]*(?:\.[A-Z0-9][A-Z0-9_-]*)+):?(\d+)?\/?/i', $this->url))
			{
				...
				if ($this->url)
				{
					// TODO add lang entry to error log lang file
					global $phpbb_log, $user;
					$phpbb_log->add('critical', $user->data['user_id'], $user->ip, 'LOG_ERROR_SFNC_ERROR_URL_NOT_VALID', false, array($this->url));
				}
			...
			else
			{
				// TODO add lang entry to error log lang file
				global $phpbb_log, $user;
				$phpbb_log->add('critical', $user->data['user_id'], $user->ip, 'LOG_ERROR_SFNC_PARSER', false, array('No data downloaded from the feed ' . $this->url));
			}
		}
		else
		{
	...
	private function init_posting()
	{
		...
		{
			global $phpbb_log;
			$phpbb_log->add('critical', $user->data['user_id'], $user->ip, 'LOG_ERROR_SFNC_ERROR_NO_POSTER_ID', false, array($this->feed_id));
			return;
		}
	...
	private function apply_template($text, $type = 'post')
	{
		...
		{
			global $phpbb_log;
			$phpbb_log->add('critical', $user->data['user_id'], $user->ip, 'LOG_ERROR_SFNC_MISSING_TEMPLATE', false, array($this->name, $text, $type));
			return;
		}
